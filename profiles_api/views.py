...
class HelloAPIView(APIView):
    ...
    def post(self,request):
        ...
        if serializer.is_valid():
            name = serializer.validated_data.get('name') #Retreives the name field
            message = f'Hello {name}'
            return Response({'message':message})
        else:
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST) #returns this request if data inputted is an error 
